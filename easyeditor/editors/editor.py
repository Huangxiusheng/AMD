...
class BaseEditor:
    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                # 使用多卡并行处理
                # device_ids = [0,1,2,3]  # 指定要使用的GPU设备ID列表
                # self.model = self.model.to(device_ids[3])  # 将模型放置在第一个GPU设备上
                # self.model = DataParallel(self.model, device_ids=device_ids)  # 使用DataParallel进行多卡并行处理
                ...
                self.model = LlamaForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map='auto')
    ...
    def edit(self,
        ...
        if summary_metrics and len(all_metrics)!=0:
            ...
            with open("result/mean_metrics_{}.json".format(args.model_type),"w", encoding='utf-8') as f: ## 设置'utf-8'编码
                f.write(json.dumps( mean_metrics  ,ensure_ascii=False  ,indent=4 ) )
