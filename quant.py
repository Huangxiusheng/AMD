...
class QuantLinear(nn.Module): 
    ...
    def forward(self, x,):
        ...
        cuda_i = 'cuda:{}'.format(torch.randint(low=0, high=4, size=(1,)).item())
        cuda_i = 'cpu'
        x = x.to(cuda_i)
        self.qweight = self.qweight.to(cuda_i)
        self.g_idx = self.g_idx.to(cuda_i)
        self.wf = self.wf.to(cuda_i)
        self.qzeros = self.qzeros.to(cuda_i)
        self.scales = self.scales.to(cuda_i)
